...
**Q: Explain an end-to-end agentic system design — including planner agents, tool orchestration, state management, memory, retry strategies, and human approval workflows.**
...
**Q: Architecture decision-making — when do you use a single agent, a multi-agent architecture, or a workflow framework like LangGraph or Semantic Kernel? Justify by complexity, scalability, and observability.**
...
**Q: Token optimization — practical techniques to reduce LLM costs, and how usage/cost is actually measured.**
...
**Q: How do you safely execute AI-generated outputs? Discuss schema validation, allow-lists, output parsing, approval workflows, confidence thresholds, and retry mechanisms — with an emphasis on production safety.**
...
**Q: How do you prevent hallucination? Discuss RAG grounding, citations, reranking, response validation, confidence scoring, and tool-based verification.**
...
**Q: AI evaluation — explain offline and online evaluation strategies, key metrics, and evaluation frameworks (LangSmith, DeepEval, Promptfoo, Azure AI Evaluations, OpenAI Evals).**
...
**Q: How do you identify scenarios requiring manual approval, and explain confidence-based escalation and workflow resumption.**
...
**Q: Design a complete agent workflow with multiple tool integrations — explaining retries, timeouts, audit logging, graceful degradation, and handling partial failures.**
...
**Q: Production readiness — observability, monitoring, logging, tracing, security, scalability, and reliability. What does *real* production experience look like, vs. a proof-of-concept?**
...
# Agentic AI Evaluation: Key Concepts & Reference Guide

## 1. Concepts & Trajectory Evaluation

### Q: What is agentic evaluation?
**A:** Agentic evaluation measures not only the final answer produced by an agent, but also whether the agent followed an appropriate sequence of actions (such as selecting the correct tools, using them correctly, and following logical steps).

### Q: What is trajectory evaluation?
**A:** Trajectory evaluation checks the complete sequence of messages, internal decisions, and tool calls an agent makes while attempting to complete a task.

### Q: What is a reference trajectory?
**A:** A reference trajectory is the expected, ideal sequence of actions that represents correct agent behavior for a given test scenario. It is typically created or approved by a human domain expert for both representative scenarios and critical edge cases.

### Q: Why isn't evaluating only the final answer enough for an agent?
**A:** Evaluating only the final answer can be misleading because an agent can produce a seemingly correct result while following a poor, insecure, or highly inefficient execution path (e.g., repeating calls unnecessarily or using unsafe tool arguments).

### Q: How does trajectory evaluation differ from using an LLM as a judge?
**A:** 
* **Trajectory evaluation:** Focuses on *how* the agent completed the task (the step-by-step path and tool usage).
* **LLM-as-a-Judge:** Focuses primarily on evaluating the quality, accuracy, and nuance of the resulting final answer.

### Q: What specific elements can we evaluate in an agent trajectory?
**A:** You can evaluate:
* **Correct tool selection** (Did it use the right tool?)
* **Tool-call order** (Did it call tools in a logical sequence?)
* **Tool arguments** (Were the parameter values accurate and safe?)
* **Missing tool calls** (Did it skip a necessary step?)
* **Extra/unnecessary tool calls** (Did it perform redundant actions?)
* **Repeated tool calls** (Is it stuck in a loop?)
* **Required step execution** (Were mandatory intermediate goals met?)

### Q: What is strict trajectory matching?
**A:** Strict trajectory matching requires the actual trajectory to match the reference trajectory closely (or exactly) in terms of the sequence, choice of tools, and tool parameters.

---

## 2. Evaluation Tooling & Infrastructure (LangSmith & OpenEvals)

### Q: What is the role of LangSmith in agent evaluation?
**A:** LangSmith provides the infrastructure needed to store evaluation datasets, run structured experiments, trace agent executions end-to-end, attach custom evaluator scores, and benchmark different versions of LLMs or agentic workflows over time.

### Q: What is the difference between Pytest and LangSmith?
**A:** 
* **Pytest:** A general-purpose Python testing framework used for unit, integration, and functional code testing.
* **LangSmith:** An evaluation and observability platform designed specifically for LLM and agent applications, focusing on tracing, prompt iteration, and model evaluation.

### Q: What is the target function in a LangSmith evaluation?
**A:** The target function is the wrapper or entry point function representing the specific application, chain, or agent being evaluated.

### Q: What is an evaluator in LangSmith?
**A:** An evaluator is a function or model that takes the application's actual output (and optionally a gold-standard reference output) and computes an evaluation result, such as a numerical score or binary pass/fail label.

### Q: What is OpenEvals?
**A:** OpenEvals is an open-source library of prebuilt evaluators designed to assess LLM and agent application outputs without needing to write custom evaluation logic from scratch.

---

## 3. Production Evaluation Framework

### Q: What agent evaluation metrics would you use in production?

Evaluating an agent in production requires a multi-dimensional approach covering execution path, answer quality, operational cost, and system reliability:
